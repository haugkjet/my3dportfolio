...
const MergedMeshes = ({ geometries, material, ...props }) => {
  ...
  export default function BarchartMerged() {
    ...
    const material = new MeshStandardMaterial({ color: currentSettings.chartcolor1,        
    ...
    const geometries = [
        new THREE.ExtrudeGeometry(shape, {depth: Math.floor(Math.random() * 5) + 1,bevelEnabled: false,
        }).translate(0,0, 0).rotateX(-Math.PI / 2),
      new THREE.ExtrudeGeometry(shape, { depth: Math.floor(Math.random() * 5) + 1, bevelEnabled: false,        
      }).translate(1.1, 0, 0).rotateX(-Math.PI / 2),       
      new THREE.ExtrudeGeometry(shape, { depth: Math.floor(Math.random() * 5) + 1, bevelEnabled: false,        
      }).translate(2.2, 0, 0).rotateX(-Math.PI / 2),       
      new THREE.ExtrudeGeometry(shape, { depth: Math.floor(Math.random() * 5) + 1, bevelEnabled: false,        
      }).translate(3.3, 0, 0).rotateX(-Math.PI / 2),       
      new THREE.ExtrudeGeometry(shape, { depth: Math.floor(Math.random() * 5) + 1, bevelEnabled: false,        
      }).translate(4.4, 0, 0).rotateX(-Math.PI / 2),       
    ...
    return (
        <>
      <MergedMeshes geometries={geometries} material={material} position={[0, 0, 0]} />
